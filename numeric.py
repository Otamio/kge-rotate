...
if __name__ == "__main__":
    ...
    command_transe = f"python codes/create_mapping.py --dataset numeric/{dataset} && " \
                     f"CUDA_VISIBLE_DEVICES={gpu} python -u codes/run.py --do_train --cuda --do_valid --do_test " \
                     f"--data_path numeric/{dataset} --model TransE -n 256 -b 1024 -d 1000 -g 24.0 -a 1.0 -adv " \
                     f"-lr 0.0001 --max_steps 150000 --valid_steps 5000 -save numeric/{dataset}_transe " \
                     "--test_batch_size 16 --use_stopper --save_best"

    command_rotate = f"CUDA_VISIBLE_DEVICES={gpu} python -u codes/run.py --do_train --cuda --do_valid --do_test " \

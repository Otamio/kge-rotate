...
if __name__ == "__main__":
    ...
    if 'FHC' in dataset or 'FHN' in dataset or 'QHC' in dataset or 'QHN' in dataset:
        for lv in [1, 2, 3, 4, 5]:
            subprocess.run(f"python ablation.py --dataset {dataset}_{lv} --gpu {gpu}", shell=True)
    else:
        for bins in [2, 4, 8, 16, 32]:
            subprocess.run(f"python ablation.py --dataset {dataset}_{bins} --gpu {gpu}", shell=True)
